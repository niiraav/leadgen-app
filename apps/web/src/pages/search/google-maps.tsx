...
export default function SearchGoogleMaps() {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          const city = data.address?.city || data.address?.town || data.address?.village || data.address?.postcode || "Unknown";
          setLocation(city);
        } catch {
          // Silently fail
        }
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const sorted = scoredResults.sort((a, b) => b.hot_score - a.hot_score);
      setResults(sorted);
      setResultsHeader({
        count: sorted.length,
      ...
      setRecentSearches((prev) => {
        const entry: RecentSearch = { businessType, location, radiusKm, filters: {}, timestamp: Date.now() };
        const filtered = prev.filter((s) => s.businessType !== businessType || s.location !== location);
  ...
  const handleAddAll = async (hotOnly: boolean) => {
    ...
    for (let i = 0; i < leads.length; i++) {
      ...
      try {
        ...
        setResults((prev) =>
          prev.map((res) => (res.title === r.title ? { ...res, added: true, id: lead.id } : res))
        );
        added++;
      } catch {
        // Skip failed
    ...
    setAddingAll("none");
    showToast(`✓ ${added} leads added — [View Pipeline](/pipeline)`);
  };
    ...
    <>
      ...
      {toast && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 w-full max-w-sm px-4">
          <div className="flex items-center gap-3 rounded-xl border border-border bg-surface px-4 py-3 shadow-xl">
            ...
            <button onClick={() => setToast(null)} className="text-text-faint hover:text-text">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      <div className="max-w-xl mx-auto pb-20 md:pb-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-text-muted mt-1">Find local businesses on Google Maps</p>
        </div>
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2 rounded-lg border border-border bg-surface-2 px-3 py-2.5 focus-within:ring-2 focus-within:ring-blue/20">
            ...
            <input
              type="text"
              value={businessType}
              onChange={(e) => { setBusinessType(e.target.value); setSelectedType(null); }}
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2 rounded-lg border border-border bg-surface-2 px-3 py-2.5 focus-within:ring-2 focus-within:ring-blue/20">
            <MapPin className={`w-4 h-4 ${geoLoading ? "animate-pulse text-blue" : "text-text-faint"}`} />
            <input
              type="text"
              value={location}
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs text-text-muted font-medium">{radiusKm} km</span>
          </div>
          <input
            type="range"
            min={1}
            ...
            <div className="space-y-1">
              {recentSearches.map((s, i) => (
                ...
                >
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); setRecentSearches((prev) => prev.filter((_, idx) => idx !== i)); }}
        ...
        {/* Results */}
        {results.length > 0 && resultsHeader && (
          <div ref={resultsRef} className="mt-8 pt-6 border-t border-border/40">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-text">
                {resultsHeader.count} results · {resultsHeader.hotCount} 🔥 hot leads
              </p>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-text-muted mt-0.5 truncate">📍 {r.address || "No address"}</p>
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-xs text-text-muted">
                        {r.rating != null && <span>⭐ {r.rating}</span>}
                        {r.reviews != null && <span>({r.reviews} reviews)</span>}
                        {r.website ? <span className="text-green">🌐 Has site</span> : <span className="text-red">🌐 No site</span>}
                      </div>
                      {r.phone && <p className="text-xs text-text-muted mt-1">📞 {r.phone}</p>}
                    </div>
                    {/* Add button */}
                    <button
                      ...
                      className={`shrink-0 w-9 h-9 rounded-lg flex items-center justify-center transition-all active:scale-90 ${
                        r.added ? "bg-green/10 text-green" : "bg-blue/10 text-blue hover:bg-blue/20"
                      }`}
                    >
                      {r.added ? <Check className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
                    </button>
                  </div>
                </div>
              ))}
