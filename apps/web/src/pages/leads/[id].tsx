...
export default function LeadProfilePage({ user }: { user?: { id: string; email: string } }) {
  ...
  if (loading) {
    return (
      <div className="space-y-6 max-w-6xl animate-pulse" aria-busy="true" aria-live="polite">
    ...
    return (
      <div className="space-y-6 max-w-3xl">
        <div className="card text-center py-12">
          <p className="text-muted-foreground" role="alert">{error || "Lead not found"}</p>
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground mt-1.5">
            {lead.city && (
              <span className="flex items-center gap-1">
                <MapPin className="w-3.5 h-3.5" aria-hidden="true" />{lead.city}, {lead.country}
              </span>
            )}
          ...
          <div className="flex items-center gap-1.5">
            <PoundSterling className="w-3.5 h-3.5 text-muted-foreground" aria-hidden="true" />
            <input
              type="number"
              min={0}
              step={100}
            ...
            />
            {saving && <Loader2 className="w-3 h-3 animate-spin text-muted-foreground" aria-hidden="true" />}
          </div>
          ...
          <div className="relative">
            <button
              ...
              aria-expanded={sequencesDropdown}
              aria-haspopup="listbox"
            >
              Enroll in Sequence
              {lead.doNotContact && <AlertTriangle className="w-3 h-3 ml-1 text-warning" aria-hidden="true" />}
      ...
      >
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <Card className="p-4 space-y-3">
            ...
            <div className="space-y-1">
              <label htmlFor="follow-up-date" className="text-micro-sm font-medium text-muted-foreground uppercase tracking-wide">Follow-up Date</label>
            ...
            {(lead.status === "lost" || lead.pipelineStage === "lost") && (
              <div className="space-y-2 pt-1 border-t border-border/30">
                <label htmlFor="loss-reason" className="text-micro-sm font-medium text-muted-foreground uppercase tracking-wide">Loss Reason</label>
                <select
                  id="loss-reason"
                ...
                <label htmlFor="loss-notes" className="sr-only">Loss notes</label>
                <textarea
                  id="loss-notes"
                  value={editLossNotes}
          ...
          <Card className="p-0">
            <div className="px-4 pt-4 pb-2 flex items-center gap-1.5">
              ...
              {lead?.review_summary && lead.reviews_fetched_at && (Date.now() - new Date(lead.reviews_fetched_at).getTime()) / 86400000 >= 7 && (lead?.place_id || lead?.business_name) && (
                ...
                >
                  {reviewsLoading ? <Loader2 className="w-3 h-3 animate-spin" aria-hidden="true" /> : <RefreshCw className="w-3 h-3" aria-hidden="true" />}
                  Refresh
                </button>
              )}
            </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {lead.email_status === "catch-all" && (
            <div className="rounded-xl border border-warning/20 bg-warning/5 p-3 text-xs text-warning" role="alert">
              ...
              emailTab={
                <div>
                  ...
                  <div className="px-4 pt-4 pb-3 border-b border-border/40">
                    ...
                    {emailError && (
                      <div className="text-xs text-destructive mb-2" role="alert">{emailError}</div>
                    )}
                  ...
                  <div className="px-4 pb-3 pt-1">
                    <label htmlFor="email-body" className="sr-only">Body</label>
                    <textarea
                      id="email-body"
                      ref={textareaRef}
                      value={draftEmail}
                        ...
                        <div className="relative group">
                          ...
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-48">
                            <div className="text-xs text-foreground bg-card border border-border rounded-lg px-3 py-2 shadow-lg">
                              <AlertCircle className="w-3 h-3 inline mr-1 text-destructive" aria-hidden="true" />
                ...
                <div className="px-4 pb-4 space-y-3">
                  {repliesQuery.isLoading ? (
                    <div className="flex items-center gap-2 text-xs text-muted-foreground py-4">
                      <Loader2 className="w-3 h-3 animate-spin" aria-hidden="true" /> Loading replies...
                    </div>
                  ) : !repliesQuery.data?.replies?.length ? (
