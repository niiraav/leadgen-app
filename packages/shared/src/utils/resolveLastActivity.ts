import type { LeadActivity, ActivityEntry, ReplyIntent } from '../types';
...
const ACTIVITY_PRIORITY: LeadActivity['type'][] = [
...
const VALID_REPLY_INTENTS: Set<string> = new Set<ReplyIntent>([
  'interested',
  'question',
  'objection',
  'not_now',
...
export function resolveLastActivity(
  ...
  if (INTENT_CARRYING_TYPES.has(best.type) && best.reply_intent && VALID_REPLY_INTENTS.has(best.reply_intent)) {
    replyIntent = best.reply_intent as ReplyIntent;
  }
